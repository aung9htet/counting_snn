...
class Implementation_Model():
    ...
    def init_parameters(self):
        ...
        self.gain_mod_parameters = {
            ...
            "tau_m": 1.6,                    # ms membrane time constant
            "C_m": 2 * 10**6,     # pF membrane capacitance (calculated for R = 1)
            "t_ref": 0                      # ms refractory period
        }
    ...
    def init_ring_attractor(self):
        ...
        distances = self.neuron_distance(self.population_size, self.max_distance)
    ...
    def connect_homeostasis_gain(self):
        ...
        for neuron in self.move_high_neurons:
            nest.Connect(self.high_neuron, neuron, syn_spec={'weight': 1.5 * 10**3})
        for neuron in self.move_low_neurons:
            nest.Connect(self.low_neuron, neuron, syn_spec={'weight': 1.5 * 10**3})
    ...
    def connect_gain_ring(self):
        ...
        for index in range(self.population_size):
            ...
            nest.Connect(self.ring_neurons[index], self.move_high_neurons[index], syn_spec={'weight': 0.9 * 10**3})
            nest.Connect(self.ring_neurons[index], self.move_low_neurons[index], syn_spec={'weight': 0.9 * 10**3})
            # Gain Mod Neuron to Ring Connection
            nest.Connect(self.move_high_neurons[index], self.ring_neurons[next_calc(index)], syn_spec={'weight': 5 * 10**3})
            nest.Connect(self.move_low_neurons[index], self.ring_neurons[prev_calc(index)], syn_spec={'weight': 5 * 10**3})

    def __init__(self, representation=None, external=None):
        """
    ...
    def neuron_distance(self, size, max_distance):
        """
    ...
    def get_homeostasis_info(self):
        """
            The following model gets the firing rate of the model for high neuron and low
            neuron
        """

        high_fr = len(self.high_spike.get("events")["times"])/10
        low_fr = len(self.low_spike.get("events")["times"])/10
        return high_fr, low_fr
    
    def get_gain_mod_info(self):
        """
            The following model gives an array of firing rate for each of the gain modulation
            neurons in the model.
        """
        gain_mod_high_fr = []
        gain_mod_low_fr = []
        for spike_recorder in self.move_high_spike_recorders:
            gain_mod_high_fr.append(len(spike_recorder.get("events")["times"])/10)
        for spike_recorder in self.move_low_spike_recorders:
            gain_mod_low_fr.append(len(spike_recorder.get("events")["times"])/10)
    ...
    def plot_homeostasis_heatmap(self):
        ...
        for external_index in range(len(external_range)):
            for representation_index in range(len(representation_range)):
                ...
                self.model = Implementation_Model()
                self.model.external_rate = external_range[external_index]
                self.model.representation_rate = representation_range[representation_index]
                self.model.run()
...
if __name__ == "__main__":
    ...
    row_2 = table_insert("phh      ", "Plot Homeostasis Heatmap")
    row_3 = table_insert("<Unknown>", "Don't Run               ")
    row_4 = table_insert("pbgm     ", "Plot Bar for Gain Mod   ")
    args_print = init_introduction + row_1 + row_2 + row_3 + row_4
    run_type = input(args_print + "\nEnter Run Type: ")
