...
class Implementation_Run():
    ...
    def full_run(self):
        ...
        for index in range(len(self.model.perirhinal_fdn_neurons)):
            ...
            timestep = []
            window_start = 0
            while (window_start + window_frame) < self.model.total_timestep: 
    ...
    def record_data(self, count_number):
        ...
        for run_count in range(21):
            ...
            self.model = Implementation_Model(representation=True, external=True, tau_gain=900)
            ...
            data_file = "model_data/900/data_spike" + str(count_number)
    ...
    def plot_voltage_gain_mod(self):
        tau_list = [900, 1000]

        fig_colors = ["#1f77b4", "#ff7f0e"]
        tau_design = [":", "-"]
        plt.figure(figsize=(14,10))
        for tau_i in range(len(tau_list)):
            ...
            for count_i in range(len(counts)):
                ...
                multimeters = self.model.move_high_multimeters
                ...
                for multimeter in multimeters:
                    ...
                    Vms = dmm["events"]["V_m"]
                    ts = dmm["events"]["times"]
                    if average_voltage is None:
                        average_voltage = Vms
                    else:
                        average_voltage = np.add(average_voltage, Vms)
                    run += 1
                average_voltage = np.divide(average_voltage,run)
                plt.plot(ts, average_voltage, tau_design[tau_i], color = fig_colors[count_i], lw=3.0)
        plt.xlabel("Timestep", fontsize=30)
        plt.ylabel("Average Potential Difference", fontsize=30)
        plt.tick_params(axis='both', which='both', labelsize=25)
        plt.show()
...
if __name__ == "__main__":
    ...
    if run_type == None:
        ...
        implement.plot_voltage_gain_mod()